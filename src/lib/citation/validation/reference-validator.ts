...
export class ReferenceValidator {
  ...
  static detectDuplicateReferences(references: Reference[]): Array<{
    ...
    for (let i = 0; i < references.length; i++) {
      ...
      if (processed.has(ref1.id)) {
        continue;
      }

      const duplicateGroup: string[] = [ref1.id];
      let reason = '';

      for (let j = i + 1; j < references.length; j++) {
        const ref2 = references[j];
        
        if (processed.has(ref2.id)) {
          continue;
        }

        // Check for exact title match
        if (ref1.title && ref2.title && 
            ref1.title.toLowerCase().trim() === ref2.title.toLowerCase().trim()) {
          duplicateGroup.push(ref2.id);
          processed.add(ref2.id);
          reason = 'identical title';
        }
        // Check for DOI match
        else if (ref1.doi && ref2.doi && ref1.doi.trim() !== '' && ref2.doi.trim() !== '' &&
                 ref1.doi.toLowerCase().trim() === ref2.doi.toLowerCase().trim()) {
          duplicateGroup.push(ref2.id);
          processed.add(ref2.id);
          reason = 'same DOI';
        }
        // Check for similar titles
        else if (ref1.title && ref2.title) {
      ...
      if (duplicateGroup.length > 1) {
        processed.add(ref1.id);
        duplicates.push({
          references: duplicateGroup,
          reason
        });
      }
    }
