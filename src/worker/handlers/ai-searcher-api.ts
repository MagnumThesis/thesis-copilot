...
} from "../lib/query-generation-engine";
...
export class AISearcherAPIHandler {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          for (const source of body.contentSources) {
            // Check cache first
            let extracted =
              this.performanceOptimizer.getCachedContentExtraction(
                body.conversationId,
                source.source,
                source.id
              );
            ...
            if (!extracted) {
              ...
              // Schedule background preloading for related content
              this.performanceOptimizer.addBackgroundTask({
                ...
                data: {
                  ...
                  sourceId: source.id,
                  extractionFn: () =>
                    this.contentEngine.extractContent({
                      source: source.source,
                      id: source.id,
                      conversationId: body.conversationId,
                    }),
          ...
          if (extractedContent.length > 0) {
            ...
            // Check cache for query generation
            let cachedQueries =
              this.performanceOptimizer.getCachedQueryGeneration(
                extractedContent,
                queryOptions
              );
            ...
            if (cachedQueries) {
              ...
              // Generate queries and cache them
              ...
              // Schedule background query generation for alternative options
              this.performanceOptimizer.addBackgroundTask({
                type: "query_generation",
                priority: "medium",
                data: {
                  content: extractedContent,
                  options: { ...queryOptions, includeAlternatives: true },
                  generationFn: () =>
                    this.queryEngine.generateQueries(extractedContent, {
                      ...queryOptions,
      ...
      try {
        ...
        const contentSources =
          body.contentSources?.map((cs) => cs.source) || [];
      ...
      try {
        ...
        if (!degradedMode && scholarResults.length > 0) {
          ...
          // Schedule background preloading for related queries
          if (generatedQueries.length > 1) {
            for (let i = 1; i < Math.min(generatedQueries.length, 3); i++) {
              const relatedQuery = generatedQueries[i];
              this.performanceOptimizer.addBackgroundTask({
                type: "search_preload",
                priority: "low",
                data: {
        ...
        scholarResults = errorHandling.fallbackResults.map((result) => ({
          ...
          year: result.publication_date
            ? parseInt(result.publication_date)
            : undefined,
      ...
      try {
        ...
        const searchResultsForLearning = searchResults.map((result) => ({
          ...
          resultYear: result.publication_date
            ? parseInt(result.publication_date)
            : undefined,
        ...
        finalResults = rankedResults.map((result) => {
          const originalResult = searchResults.find(
            (sr) => sr.title === result.resultTitle
          );
          return {
        ...
        console.log(
          `Applied learning-based ranking for user ${userId}: ${rankedResults.length} results`
        );
      } catch (learningError) {
        console.warn(
          "Failed to apply learning-based ranking, using original results:",
          learningError
        );
      ...
      if (finalResults.length > 10) {
        progressiveLoadingState =
          this.performanceOptimizer.initializeProgressiveLoading(
            sessionId || crypto.randomUUID(),
            finalResults.length,
            10 // Load 10 results at a time
          );
      ...
      return c.json({
        ...
        generatedQueries:
          generatedQueries.length > 0 ? generatedQueries : undefined,
        extractedContent:
          extractedContent.length > 0 ? extractedContent : undefined,
      ...
      try {
        if (body.type === "doi" || body.source.includes("doi.org")) {
          ...
          if (results && results.length > 0) {
            ...
            extractedMetadata = {
              ...
              journal: result.journal,
              publication_date:
                result.publication_date || result.year?.toString(),
              doi: result.doi || doi,
          ...
          if (possibleTitle.length > 3) {
            ...
            if (results && results.length > 0) {
              ...
              extractedMetadata = {
                ...
                publication_date:
                  bestResult.publication_date || bestResult.year?.toString(),
                doi: bestResult.doi,
        ...
        if (!extractedMetadata) {
          extractedMetadata = {
            ...
            relevance_score: 0.1,
            abstract:
              "Metadata extraction failed. Please verify the source and try again.",
            keywords: [],
          };
        }
  ...
  async getHistory(c: Context<AISearcherContext>) {
    ...
    try {
      ...
      if (contentSources && contentSources.length > 0)
        filter.contentSources = contentSources;
  ...
  /**
   * GET /api/ai-searcher/results/next-batch
   * Get next batch of results for progressive loading
   */
  async getNextBatch(c: Context<AISearcherContext>) {
    const startTime = Date.now();

    try {
      const sessionId = c.req.query("sessionId");
      const searchSessionId = c.req.query("searchSessionId");
      ...
      return c.json({
        success: true,
        results: nextBatch,
        loaded_results: nextBatch.length,
      ...
      return c.json(
        {
          success: false,
          error: "Failed to get next batch of results",
          processingTime: Date.now() - startTime,
        },
        500
      );
  ...
  /**
   * GET /api/ai-searcher/performance/metrics
   * Get performance metrics and cache statistics
   */
  async getPerformanceMetrics(c: Context<AISearcherContext>) {
    const startTime = Date.now();
  ...
  /**
   * POST /api/ai-searcher/performance/clear-cache
   * Clear performance caches
   */
  async clearCache(c: Context<AISearcherContext>) {
    const startTime = Date.now();
  ...
  async getSearchSessionDetails(c: Context<AISearcherContext>) {
    ...
    try {
      ...
      const sessionDetails = await historyManager.getSearchSessionDetails(
        sessionId
      );
    ...
    try {
      ...
      for (const source of body.contentSources) {
        try {
          ...
          console.warn(
            `Failed to extract content from ${source.source}:${source.id}:`,
            error
          );
      ...
      // Generate queries
      const queries = this.queryEngine.generateQueries(
        extractedContents,
        body.options
      );
    ...
    try {
      ...
      for (const source of body.sources) {
        try {
          ...
          console.error(
            `Error extracting content from ${source.source}:${source.id}:`,
            error
          );
          errors.push({
            source: source.source,
  ...
  async contentPreview(c: Context<AISearcherContext>) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `Error previewing content from ${body.source}:${body.id}:`,
          extractionError
        );
  ...
  async refineQuery(c: Context<AISearcherContext>) {
    ...
    try {
      ...
      const refinement = this.queryEngine.refineQuery(
        body.query,
        originalContent
      );
    ...
    if (result.journal) {
      ...
      if (
        highQualityJournals.some((journal) =>
          result.journal!.toLowerCase().includes(journal.toLowerCase())
        )
      ) {
        citations += 25;
      }
...
app.get("/history/content-usage", (c) =>
  aiSearcherAPIHandler.getContentUsage(c)
);
...
app.get("/analytics/query-performance", (c) =>
  aiSearcherAPIHandler.getQueryPerformance(c)
);
app.get("/analytics/success-rate-tracking", (c) =>
  aiSearcherAPIHandler.getSuccessRateTracking(c)
);
app.post("/analytics/success-tracking", (c) =>
  aiSearcherAPIHandler.getSuccessTracking(c)
);
app.get("/analytics/content-source-effectiveness", (c) =>
  aiSearcherAPIHandler.getContentSourceEffectiveness(c)
);
app.get("/session/:sessionId", (c) =>
  aiSearcherAPIHandler.getSearchSessionDetails(c)
);
app.post("/track-result-action", (c) =>
  aiSearcherAPIHandler.trackResultAction(c)
);
